...
class TodoElement extends React.Component {
    ...
    mountTodo() {
        ...
        if (deadlineType === 'deadline') {
            this.text = lang['todo-type-deadline'] + ' ' + DateToFormatString(GetDate(Deadline));
        } else if (deadlineType === 'schedule') {
            const nextDate = GetDate(now + (minDeltaDays * 24 * 60 * 60));
            this.text = lang['todo-type-repeat-before'] + ' ' + DateToFormatString(nextDate);
        }
    ...
    onCheck = () => {
        ...
        const { todo, onTodoCheck } = this.props;

        onTodoCheck(todo, (resolve) => {
    ...
    render() {
        ...
        const { Title, Schedule, Checked } = todo;
        ...
        const openTodo = () => user.interface.ChangePage('todo', { todo });
