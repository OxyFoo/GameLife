...
class Button extends ButtonBack {
    ...
    renderBackground = () => {
        const { appearance, color, borderColor, styleBackground, gradientColors, gradientColorsAngle } = this.props;

        if (appearance === 'normal') {
            return <Gradient containerStyle={styles.absolute} colors={gradientColors} angle={gradientColorsAngle} />;
            ...
            const backgroundColor = themeManager.GetColor('darkBlue', { opacity: 0.7 });
            return (
                <>
                    <View style={[styles.absolute, styles.backgroundBlur, { backgroundColor }]} />
                    <MaskedView
                        style={styles.absolute}
                        maskElement={<View style={[styles.backgroundView, styleBackground]} />}
                    >
