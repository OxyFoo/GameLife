...
function RadialBackground(props) {
    ...
    React.useEffect(() => {
        if (!!props.animPath) {
            Animated.loop(
                Animated.sequence(props.animPath.map((pos, i) =>
                    TimingAnimation(
                        animPos,
                        { x: pos.x * win.width, y: pos.y * win.height },
            ...
            <Svg style={styles.radial}>
                <Defs>
                    <RadialGradient id="grad">
                        <Stop offset="0%" stopColor={themeManager.GetColor(props?.color || 'main1')} stopOpacity=".2" />
