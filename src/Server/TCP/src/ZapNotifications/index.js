 ...
 * @property {string} [callbackID]
 * 
 * @typedef {Object} ReceiveDiscordZapUsersStatsRequest
 * @property {string} token Simple authentication to prevent unauthorized requests
 * @property {'get-users-stats'} command
 * @property {string} [callbackID]
 * 
 * @typedef {Object} SendDiscordZapUsersStatsRequest
 * @property {'sent'} status
 * @property {Object} stats
 * @property {number} stats.usersCount
 * @property {string} [callbackID]
 */
 ...
 * @param {ReceiveDiscordZapNotificationRequest | ReceiveDiscordZapUsersStatsRequest} data
 * @returns {Promise<boolean>}
 */
async function ZapCommand(users, connection, data) {
    if (data.command === 'send-global-request') {
        return await SendGlobalRequest(users, connection, data);
    } else if (data.command === 'get-users-stats') {
        return await SendGlobalResponse(users, connection, data);
    }
 ...
 * @returns {Promise<boolean>}
 */
async function SendGlobalRequest(users, connection, data) {
    // Add request in logs
...
* @param {Users} users
* @param {connection} connection
* @param {ReceiveDiscordZapUsersStatsRequest} data
* @returns {Promise<boolean>}
*/
async function SendGlobalResponse(users, connection, data) {
    const stats = await GetStats(users);

    /** @type {SendDiscordZapUsersStatsRequest} */
    const response = {
        status: 'sent',
        stats: stats,
        callbackID: data.callbackID
    };
    SendCallback(connection, response);
...
/**
 * @param {connection} connection
 * @param {SendDiscordZapNotificationRequest | SendDiscordZapUsersStatsRequest} data
 */
