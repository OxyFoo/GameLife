    ...
    renderItem = ({ item: index }) => {
        return (
            <SvgUri
                ...
                //source={require('./homer.svg')}
                //source={{uri:'http://thenewcode.com/assets/images/thumbnails/homer-simpson.svg'}}
            />
        );
    }
    render() {
        const svgs = Array(this.state.testInt).fill(0).map((_, i) => {
            return (
                <SvgUri
                    key={'svg-' + i.toString()}
                    width="20"
        ...
        return (
            <Page ref={ref => this.refPage = ref} canScrollOver>
                {/*<FlatList
                    data={Array(this.state.testInt).fill(0).map((_, i) => i)}
                    numColumns={15}
                    renderItem={this.renderItem}
                    keyExtractor={item => 'svg-' + item.toString()}
                />*/}
                {svgs}
                <Button color='main2' style={{ marginTop: 12 }} onPress={this.reload}>{'Reload'}</Button>
            </Page>
        );
        return (
